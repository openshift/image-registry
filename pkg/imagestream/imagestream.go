...
func (is *imageStream) localRegistry(ctx context.Context) ([]string, *rerrors.Error) {
	...
	if err != nil {
		context.GetLogger(ctx).Warnf("localRegistry: unable to parse dockerImageRepository %q", stream.Status.DockerImageRepository)
	}
	...
	if err != nil {
		context.GetLogger(ctx).Warnf("localRegistry: unable to parse publicDockerImageRepository %q", stream.Status.DockerImageRepository)
	}
